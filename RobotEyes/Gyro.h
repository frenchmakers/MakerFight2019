...
//
int MPU6050_read(int addr, int start, uint8_t *buffer, int size)
{
  int i, n, error;
...
//
int MPU6050_write(int addr, int start, const uint8_t *pData, int size)
{
  int n, error;
...
//
int MPU6050_write_reg(int addr, int reg, uint8_t data)
{
  int error;

  error = MPU6050_write(addr, reg, &data, 1);
...
int read_gyro_accel_vals(int addr, uint8_t* accel_t_gyro_ptr) {
  // Read the raw values.
  ...
  int error = MPU6050_read (addr, MPU6050_ACCEL_XOUT_H, (uint8_t *) accel_t_gyro, sizeof(*accel_t_gyro));
...
void calibrate_sensors(int addr) {
  int                   num_readings = 10;
  float                 x_accel = 0;
  float                 y_accel = 0;
  float                 z_accel = 0;
  ...
  read_gyro_accel_vals(addr, (uint8_t *) &accel_t_gyro);
